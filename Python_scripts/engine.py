...
def train(model: torch.nn.Module,
    ...
    print(
        ...
        f"Train Loss: {results['train_loss'][-1]:.4f} | Acc: {results['train_acc'][-1]:.4f} | "
        f"Prec: {results['train_precision'][-1]:.4f} | Recall: {results['train_recall'][-1]:.4f} | "
        f"F1: {results['train_f1'][-1]:.4f} | ROC-AUC: {results['train_roc_auc'][-1]:.4f} || "
        f"Test Loss: {results['test_loss'][-1]:.4f} | Acc: {results['test_acc'][-1]:.4f} | "
        f"Prec: {results['test_precision'][-1]:.4f} | Recall: {results['test_recall'][-1]:.4f} | "
        f"F1: {results['test_f1'][-1]:.4f} | ROC-AUC: {results['test_roc_auc'][-1]:.4f}"
    )
    return results
